...
- Desktop command processing now accepts server `RENAME` commands as proposal generation only: the agent validates the managed-root binding, source path, filename, symlink/reparse safety, and destination conflict, then submits a single `MOVE` proposal without touching the file before user approval.
...
- 서버 `RENAME` command를 직접 파일 변경이 아닌 같은 부모 폴더 대상 `MOVE` proposal로 변환하는 Desktop processor 경로
