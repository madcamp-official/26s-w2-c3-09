...
- Desktop command processing now accepts server `RENAME` and `MOVE` commands as proposal generation only: the agent validates the managed-root binding, source paths, filenames/destinations, symlink/reparse safety, and destination conflicts, then submits `MOVE` proposal items without touching files before user approval.
...
- 서버 `RENAME`·`MOVE` command를 직접 파일 변경이 아닌 승인 대기 `MOVE` proposal로 변환하는 Desktop processor 경로
