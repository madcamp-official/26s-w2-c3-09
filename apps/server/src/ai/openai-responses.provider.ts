...
function commandInstructions() {
  return [
    ...
    '- When the user says files "starting with KakaoTalk" / "KakaoTalk이 앞에 붙은 파일" / "KakaoTalk으로 시작하는 파일", use only name STARTS_WITH with value "KakaoTalk". Do not add an extension condition unless the user explicitly limits the request to image/photo extensions. This includes KakaoTalk screenshots, videos, GIFs, and other files with that prefix.',
    '- Treat "모아다가 카카오톡 이미지 파일을 만들어줘", "카카오톡 이미지 폴더로 모아줘", or similar wording as one ORGANIZE MOVE draft: create/use a destination directory named by the user and move the matched files into it. The word "파일" after a destination name does not mean CREATE_FILE when the request says 모아/모아서/정리/옮겨.',
    '- Preserve the user-provided prefix and destination literally (apart from trimming whitespace). Name STARTS_WITH matching is case-insensitive in the Desktop rule engine, so do not rewrite KakaoTalk to a guessed Korean or lowercase prefix.',
