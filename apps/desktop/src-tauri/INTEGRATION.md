...
`start_watching_root` registers a `notify`-based recursive watcher on the managed root, debounced 500ms so a burst of filesystem events collapses into one notification. On a debounced change it emits a `managed-root-changed` event (payload: `root_id`) that the frontend listens for to refresh Browse/History without a manual click. Changes confined to `.mousekeeper` (journal writes, our own bookkeeping) are filtered out so the app does not react to its own mutations. `WatcherStore` (in `src/storage/watchers.rs`) holds one active watcher per `root_id`; starting a new watcher for the same root replaces (and stops) the previous one, and stopping/dropping an entry stops the underlying OS watch via `RootWatcher`'s `Drop` impl. Unlike the file-engine commands, this module has no non-`tauri-commands` variant — it only exists to bridge real filesystem events to a running app, so it is gated out entirely under the default feature and tested at the `watch_root`/`WatcherStore` level instead.

Window permissions are split between the `main` app window and the `character-overlay` window. `apps/desktop/src-tauri/capabilities/default.json` grants dialog/autostart permissions only to `main`, while `capabilities/character-overlay.json` keeps the overlay on core permissions only. High-risk commands also check the caller window label in Rust and return `FORBIDDEN_WINDOW` unless invoked by `main`: managed-root registration/state updates, demo-root preparation, auto-approval changes, proposal auto-approval, proposal execution, direct trash/create/rename, undo/recovery, smart-cache processing/preferences, and watcher controls. The overlay remains a character/status/chat surface; it must hand file-related intent to the proposal-bound flow instead of invoking file operations.
...
- History is loaded from the root's `.mousekeeper/journal.jsonl` through `list_operation_history`.
