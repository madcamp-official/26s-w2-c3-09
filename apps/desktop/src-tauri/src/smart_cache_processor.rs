...
pub async fn process_smart_cache_for_room(
    ...
    let policy = agent
        .smart_cache_policy(room_id.clone())
        .await
    ...
    let key = candidate_batch_idempotency_key(&room_id, &server_candidates)?;
    let submitted_count = server_candidates.len();
    let batch = agent
        .submit_smart_cache_candidates(key, room_id, server_candidates)
        .await
        .map_err(|error| error.to_string())?;
