...
async fn run_background_tick(
    ...
    // Holds this pass's cleanup so the autonomous mobile submission can run *after* the heartbeat
    // confirms we are online, rather than issuing network calls while offline.
    ...
    if let Some(cleanup) = pending_auto_cleanup {
        let auto = crate::auto_proposal::submit_autonomous_proposals(
            &agent,
            &roots,
            &auto_approval,
            &cleanup,
