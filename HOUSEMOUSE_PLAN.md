...
- v1.3 명칭 변경: 서비스 가제를 **집쥐인(HOUSEMOUSE)**으로 변경하고 저장소·휴지통·내부 지표 식별자를 `housemouse`로 통일
...
이 문서는 집쥐인(HOUSEMOUSE) MVP를 두 명이 병렬로 개발할 수 있도록 다음을 확정한다.
...
- 서비스 가제는 **집쥐인(HOUSEMOUSE)**으로 통일한다.
- 사용자에게 노출되는 한국어 이름은 **집쥐인**, 영문 브랜드명은 **HOUSEMOUSE**를 사용한다.
- 저장소·패키지·내부 지표 등 코드 식별자는 소문자 `housemouse`를 사용한다.
- 관리자 전용 임시 휴지통 경로는 `.housemouse_trash`로 통일한다.
...
**집쥐인(HOUSEMOUSE)**은 폴더를 방으로 표현하고, 사용자가 모바일에서 정리 요청을 보내면 PC의 데스크톱 에이전트가 로컬 파일을 분석해 안전한 변경안을 제안하는 크로스플랫폼 파일 관리 서비스다.
...
8. 파일을 .housemouse_trash 또는 지정 폴더로 안전하게 이동한다.
...
| 휴지통 | `.housemouse_trash` | 원래 위치와 복구 정보를 남기고 되돌리기 가능 |
...
스마트 캐시의 기본 제외 대상은 `.housemouse_trash`, 숨김·임시 파일, credential/key 파일, 앱 DB, lock 파일이다. 확장자만으로 민감도를 단정하지 않으며 사용자는 방별 캐시 기능을 끄거나 개별 파일을 제외·고정할 수 있어야 한다.
...
- 전체 scan은 PC당 하나만 실행
- `.housemouse_trash`와 앱 내부 metadata 경로는 index에서 제외
...
## 6.5 `.housemouse_trash` 정책

```text
<managed-root>/.housemouse_trash/<operation-id>/<original-relative-path>
```
...
+ 0.30 × normalized_housemouse_open_count_30d
...
```text
desktop heartbeat interval: 15초
presence TTL: 45초
```
...
| POST | `/v1/pairing-sessions/:code/claim` | Mobile | 로그인 사용자와 PC 연결 |
...
6. online/offline 표현
7. chat command UI
8. online Files browser와 다운로드 UX
