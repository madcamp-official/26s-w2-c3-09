...
이 문서는 MOUSEKEEPER API와 object lifecycle worker를 AWS EC2에 배포하는 절차다. 실제 secret, Firebase 서비스 계정 원문과 S3 credential은 Git이나 명령 기록에 넣지 않는다.
...
```bash
sudo useradd --system --home /opt/mousekeeper --shell /usr/sbin/nologin mousekeeper
sudo install -d -o mousekeeper -g mousekeeper -m 0750 /opt/mousekeeper
sudo install -d -o root -g mousekeeper -m 0750 /etc/mousekeeper
sudo install -o root -g mousekeeper -m 0640 /dev/null /etc/mousekeeper/server.env
```

Node.js 24 LTS, Corepack과 `pnpm@11.11.0`, Git, Nginx, Certbot을 설치한다. 저장소는 `/opt/mousekeeper`에 checkout하고 `mousekeeper` 사용자가 읽을 수 있게 한다.

현재처럼 비어 있는 EC2에는 `infra/aws/bootstrap-ec2.sh`를 복사해 실행할 수 있다. 이 스크립트는 1GB 인스턴스를 위한 2GB swap, loopback 전용 PostgreSQL·Valkey Docker, Node/pnpm, migration, server systemd, Nginx와 Certbot을 구성한다. Firebase service account는 실행 전에 `/tmp/mousekeeper-firebase-service-account.json`에 권한을 제한해 전송해야 한다. Object storage가 없으면 worker를 시작하지 않고 `UNCONFIGURED`로 남긴다.

환경 변수 이름은 루트 `.env.example`을 기준으로 `/etc/mousekeeper/server.env`에 입력한다. 파일에는 실제 값이 필요하지만 Git에는 추가하지 않는다. AWS S3와 IAM role을 사용할 때는 다음 규칙을 따른다.
...
```bash
cd /opt/mousekeeper
sudo -u mousekeeper pnpm install --frozen-lockfile
sudo -u mousekeeper pnpm --filter @mousekeeper/contracts build
sudo -u mousekeeper pnpm --filter @mousekeeper/database build
sudo -u mousekeeper env NODE_OPTIONS=--max-old-space-size=1024 \
  pnpm --filter @mousekeeper/server build

sudo -u mousekeeper bash -c 'set -a; source /etc/mousekeeper/server.env; set +a; pnpm --filter @mousekeeper/database db:migrate'
```
...
```bash
sudo cp infra/aws/systemd/mousekeeper-server.service /etc/systemd/system/
sudo cp infra/aws/systemd/mousekeeper-worker.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now mousekeeper-server
sudo systemctl status mousekeeper-server --no-pager
...
```bash
sudo systemctl enable --now mousekeeper-worker
sudo systemctl status mousekeeper-worker --no-pager
```

Bootstrap은 PostgreSQL 일일 backup timer도 설치한다. 배포 후 첫 backup과 격리된 임시 DB restore drill을 실행해 dump가 실제로 복원되는지 확인한다.

```bash
sudo systemctl enable --now mousekeeper-postgres-backup.timer
...
```bash
sudo journalctl -u mousekeeper-server -u mousekeeper-worker -n 100 --no-pager
```

## 5. Nginx와 HTTPS

```bash
sudo cp infra/aws/nginx/mousekeeper.conf /etc/nginx/sites-available/mousekeeper
sudo ln -sfn /etc/nginx/sites-available/mousekeeper /etc/nginx/sites-enabled/mousekeeper
...
sudo systemctl restart mousekeeper-worker
