...
| **Offline Smart Cache 서버/모바일 control plane** | 자동 policy, candidate 제출/reservation/idempotent complete-upload/cancel API (`smart-cache.controller.ts`), room 삭제·device-revoke 시 삭제 job 큐잉, 모바일 freshness label UI (`smart_cache_page.dart`) |
