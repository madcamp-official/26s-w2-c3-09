...
- **P1 — 사용 빈도 기반 스마트 캐시:** 인프라가 구성된 환경에서는 자동으로 활성화되며, 자주 요청하거나 집쥐인을 통해 자주 열어 본 파일을 우선 선정해 암호화된 원본 일부를 서버 object storage에 저장한다.
...
```text
1. 서버가 방별 기본 quota 정책을 자동 생성하고 스마트 캐시를 활성화한다.
...
Object Storage = 만료형 transfer blob과 자동 스마트 캐시 blob
...
| 테이블 | 핵심 필드 | 설명 |
|---|---|---|
| `smart_cache_policies` | `room_id UNIQUE`, `enabled`, `quota_bytes`, `max_file_bytes`, `excluded_patterns`, `updated_at` | 서버가 자동 생성하는 방별 quota 정책 |
...
- 파일 원문은 P0 만료형 transfer 또는 P1 quota 제한 스마트 캐시 외에는 서버에 업로드하지 않음
...
- device revoke, account deletion, room removal, 버전 교체 시 object 삭제를 보장하는 tombstone job 사용
...
- 스마트 캐시 PR에는 자동 활성화, quota, stale 표시, 삭제 보장 테스트 포함
...
- P1은 서버 설정 완료 시 자동 활성화되며 캐시 시각·최신 확인 시각·availability/freshness 상태를 분리 표시함
...
- P1 room 삭제·device revoke·버전 교체 후 object 삭제를 확인할 수 있음
...
23. device revoke 또는 room 삭제 뒤 P1 object 삭제가 실행되지 않는다.
...
- P1 스마트 캐시는 정상 구성된 운영 환경에서 자동 활성화한다. 전역 feature flag는 장애 대응용 kill switch로만 사용한다.
