...
- [~] **스마트 캐시 암호화:** Desktop `smart_cache_processor`는 업로드 전 AES-256-GCM envelope(`MKS1_NONCE_CIPHERTEXT_TAG`)를 만들고 OS 보안 저장소의 smart-cache key를 사용해 signed PUT에는 암호문만 보낸다. 서버는 암호문 size/checksum과 encryption metadata를 DB에 저장·반환한다. 모바일은 key store가 실제 key를 제공할 때 ciphertext SHA-256, AES-GCM tag/nonce, plaintext SHA-256/size를 검증한 뒤 `.part` + no-overwrite로 저장한다. 실제 key sync provider는 아직 없으므로 기본 경로는 `UNCONFIGURED: SMART_CACHE_DECRYPTION_KEY_SYNC`로 fail-closed 처리한다.
...
- [~] 캐시 업로드 metadata를 암호문 size/checksum 기준으로 맞췄고 서버가 저장·반환한다. Mobile AES-GCM tag/nonce/plaintext 검증과 안전 저장 경로는 구현됐지만, 실제 key sync provider는 아직 `UNCONFIGURED` 상태다.
...
1. 스마트 캐시를 기본 `false`로 유지한 채 Mobile 복호화 key sync provider를 설계·연결하고, 이미 구현된 AES-GCM tag 검증/안전한 로컬 plaintext 저장 경로를 실제 key delivery와 E2E로 검증한다.
...
## 2026-07-14 Smart-cache encrypted mobile download

- [x] Mobile smart-cache encrypted download now has a real verification path behind key sync: ciphertext SHA-256 check, `MKS1_NONCE_CIPHERTEXT_TAG` envelope parsing, matching 32-byte key validation, AES-256-GCM auth tag and nonce verification, plaintext SHA-256/size verification from server metadata, and `.part` + no-overwrite local handoff.
- [x] The default mobile key store still returns `UNCONFIGURED: SMART_CACHE_DECRYPTION_KEY_SYNC`, so encrypted smart-cache does not create fake plaintext success before an actual desktop-to-mobile key-sync provider is designed.
- [x] Release preflight `-RunLocalChecks` now includes the focused mobile smart-cache encrypted download, verified download, and owner-scoped display-cache tests.

## 2026-07-14 Desktop smart-cache policy exclusions

- [x] Desktop smart-cache candidate processing now applies room policy `excludedPatterns` before file validation and server batch submission. The matcher mirrors the server glob contract for `**`, `*`, and `?`, counts policy-filtered files as skipped, and still leaves the server-side policy check as the authoritative safety boundary.
- [x] Smart-cache policy now includes `pinnedPatterns`. Mobile sends pinned glob rules from the policy dialog, the server persists them through Drizzle migration `0024_luxuriant_gambit.sql`, and Desktop maps matching candidates to `manualPin=true` before reservation submission.
